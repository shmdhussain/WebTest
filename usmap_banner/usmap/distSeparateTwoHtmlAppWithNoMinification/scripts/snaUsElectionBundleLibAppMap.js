...
// ┌─────────────────────────────────────────────────────────────────────┐ \\
// │ Raphaël 2.0 - JavaScript Vector Libraryy                            │ \\
// ├─────────────────────────────────────────────────────────────────────┤ \\
...
(function($, document, window, Raphael, undefined) {
  ...
  function jQueryPluginFactory( $, name, methods, getters ){
    ...
    $.fn[name] = function(){
      ...
      this.each(function() {
        ...
        } else if(typeof args[0] == 'string' && args[0] == 'getOptions'){
          returnValue = plugin.options;
        } else if(typeof args[0] == 'string' && args[0] == 'setOptions' && typeof args[1] == 'object'){
          plugin.options = args[1];
        } else if(typeof args[0] == 'string' && args[0] == 'getPluginObject'){
          returnValue = plugin;
        }
  ...
  var defaults = {
    ...
    'stateSpecificTopTextStyles': {},
    'stateSpecificTopTextCountStyles': {},
  ...
  var methods = {
    ...
    _init: function(options) {
      ...
      this.paper.setViewBox(0, 0, paperWidthWithLabels, HEIGHT, true);
    ...
    _initCreateStates: function() {
      ...
      this.neStates = ['MA', 'RI', 'CT', 'NJ', 'DE', 'MD', 'DC'];
      this.states = {
        AK: {
          ElectoralCounts: 3,
          yOffset: 2.8
      ...
      var stateAttr = {}, textObjCreated, textObj, textObjCount;
      var topTextStylesAttr      = { 'font-size': 12, fill: '#FFFFFF', stroke: 'none', 'font-family': 'Arial,Helvetica,sans-serif'};
      var topTextCountStylesAttr = { 'font-size': 11, fill: '#FFFFFF', stroke: 'none', 'font-family': 'Arial,Helvetica,sans-serif'};
...
//          console.log(attr)
      ...
      "stroke-width": 0, "opacity" : 0.0, 'cursor': 'pointer'});
        ...
        //Set classes for state and hover state.
        this.stateShapes[state].node.setAttribute("class",state);;
        this.stateHitAreas[state].node.setAttribute("class",state + "_hover");;

        //render the state code and elctroCount on top of state
        if (this.neStates.indexOf(state) <= -1) {
          if(this.options.stateSpecificTopTextStyles[state]) {
            $.extend(topTextStylesAttr, this.options.stateSpecificTopTextStyles[state]);
            $.extend(topTextCountStylesAttr, this.options.stateSpecificTopTextCountStyles[state]);
          }
           textObjCreated = this.labelPath(this.stateHitAreas[state], state, topTextStylesAttr, topTextCountStylesAttr);
           textObj = textObjCreated.textObj;
           textObjCount = textObjCreated.textObjCount;

           this.stateText[state] = textObj;
           this.stateTextCount[state] = textObjCount;
            if (typeof this.stateText[state].node.childNodes != 'undefined' && this.stateText[state].node.childNodes.length > 0) {
              this.stateText[state].node.childNodes[0].dataState = state;
              this.stateTextCount[state].node.childNodes[0].dataState = state;
            }
        }
      }
      ...
      for(var state in this.stateHitAreas) {
        ...
        if (typeof this.stateText[state]!= 'undefined') {
          $(this.stateText[state].node).bind('click', this._onClickProxy);
        }
      }
    },
    ...
    /**
     * Create the labels for the states
     */
    labelPath: function ( pathObj, text, textattr, textattrCount) {
      var bbox = pathObj.getBBox();
      var xOffset = typeof this.states[text] !== 'undefined' && typeof this.states[text].xOffset !== 'undefined' ? this.states[text].xOffset : 2;
      var yOffset = typeof this.states[text] !== 'undefined' && typeof this.states[text].yOffset !== 'undefined' ? this.states[text].yOffset : 2;

      var textObj = pathObj.paper.text( bbox.x + bbox.width / xOffset, bbox.y + bbox.height / yOffset, text).attr( textattr );
      var textObjCount = pathObj.paper.text( bbox.x + bbox.width / xOffset, bbox.y + (bbox.height / yOffset) + 12, typeof this.states[text] !== 'undefined' ? this.states[text].ElectoralCounts : '..').attr( textattrCount );
      textObj.node.setAttribute("class", text + "_text");
      textObjCount.node.setAttribute("class", text + "_text_count");
      return {
          'textObj': textObj, 
          'textObjCount': textObjCount
      };
    },
    ...
    _initCreateLabels: function() {
      ...
      var downBy = (oHeight+oGap)/this.scale;
      ...
      for(var i=0, x, y, state; i<neStates.length; ++i) {
        ...
        this.labelShapes[state].node.setAttribute("class", state);
        ...
        // add the text
        this.labelTexts[state] = R.text(x+(shapeWidth/2), y+(shapeHeight/2), state + ' ' + this.states[state].ElectoralCounts).attr(stateAttr);
        ...
        this.labelHitAreas[state].node.setAttribute("class", state);
      }
